...
- You should write the detailed version information to **`.qmake.conf`** and save it to the root directory of this repository. qmake will automatically copy FFmpeg run-time libraries if you set the **`ffmpeg_dir`** variable and remember to use double quotation marks if there are any whitespaces in it. Using **`/`** instead of **`\`** is highly recommended as well. Example:
   ```text
   DD_MAJOR_VERSION = 2
   DD_MINOR_VERSION = 3
   DD_PATCH_VERSION = 4
   DD_BUILD_VERSION = 5
   ffmpeg_dir = "D:/sample code/ffmpeg/bin/x86"
   ```
