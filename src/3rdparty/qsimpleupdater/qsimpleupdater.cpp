...
{
    return getUpdater (url)->isUpdateAvailable();
}
...
void QSimpleUpdater::setAutoUpdate(const QString& url, bool enabled)
{
    getUpdater (url)->setAutoUpdate(enabled);
}
...
Updater* QSimpleUpdater::getUpdater (const QString& url) const
{
    if (!URLS.contains (url)) {
        ...
        connect (updater, &Updater::updateAvailable,
                 this,    &QSimpleUpdater::updateAvailable);
