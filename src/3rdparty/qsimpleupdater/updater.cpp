...
Updater::Updater()
{
    ...
    setUserAgentString (QStringLiteral ("%1/%2 (Qt; QSimpleUpdater)").arg (qApp->applicationName(),
...
void Updater::setAutoUpdate(bool enabled)
{
...
void Updater::onReply (QNetworkReply* reply)
{
    ...
    QJsonObject updates = document.object().value (QStringLiteral("updates")).toObject();
    QJsonObject platform = updates.value (platformKey()).toObject();

    /* Get update information */
    m_openUrl = platform.value (QStringLiteral("open-url")).toString();
    m_changelog = platform.value (QStringLiteral("changelog")).toString();
    m_downloadUrl = platform.value (QStringLiteral("download-url")).toString();
    m_latestVersion = platform.value (QStringLiteral("latest-version")).toString();

    /* Compare latest and current version */
    int suffixIndex;
    QVersionNumber remoteVersion = QVersionNumber::fromString(latestVersion(), &suffixIndex);
    QVersionNumber localVersion = QVersionNumber::fromString(moduleVersion(), &suffixIndex);
    setUpdateAvailable (remoteVersion > localVersion);
    emit checkingFinished (url());
}
...
void Updater::setUpdateAvailable (const bool available)
{
    ...
    if (isUpdateAvailable() && (notifyOnUpdate() || notifyOnFinish())) {
        ...
        if (exec == QMessageBox::Yes) {
            if (!openUrl().isEmpty())
                ...
                m_downloader->setFileName (downloadUrl().split (QLatin1Char('/')).last());
    ...
    else if (!autoUpdate() && notifyOnFinish()) {
